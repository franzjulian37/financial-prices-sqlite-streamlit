...
# Se il CSV è nella stessa cartella dello script, puoi usare questo:
METRICS_CSV = CURRENT_DIR / "scripts" / "metrics.csv"
...
st.markdown("Mostro i dati calcolati dal tuo script (`metrics.csv`). Usa la sidebar per scegliere ticker e intervallo.")

# --- Sidebar: filtri ---
with st.sidebar:
    st.header("Filtri")
    if df.empty:
        st.warning("File metrics.csv non trovato. Esegui `python scripts/compute_metrics.py` prima di avviare Streamlit.")
    else:
